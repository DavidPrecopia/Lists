...
class AddEditUserListLogicTest {
    ...
    private val logic = AddEditUserListLogic(view, viewModel, repo, id, title, position)
    ...
    inner class Save {
        ...
        @Test
        fun `save - Add`() {
            val capturedArg = CapturingSlot<UserList>()

            every { viewModel.taskType } returns ADD
            every { viewModel.position } returns position
            every { repo.addUserList(userList = capture(capturedArg)) } answers { Unit }
    ...
    inner class ValidateInput {
        ...
        /**
         * - Check if the input is different from the current title.
